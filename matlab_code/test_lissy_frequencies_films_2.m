...
error_obs_proj_RK    = zeros(2*Nx,Nt);
error_obs_proj_RK_bf = zeros(2*Nx,Nt);

for t=1:Nt
    error_obs_proj_RK(:,t) =    PN * error_obs_RK(:,t);
    error_obs_proj_RK_bf(:,t) = PN * error_obs_RK_bf(:,t);
end

norm_error_state_RK = vecnorm(error_state_RK);
norm_error_obs_proj_RK = vecnorm(error_obs_proj_RK);
norm_error_obs_proj_RK_bf = vecnorm(error_obs_proj_RK_bf);
...
figure(1)
semilogy(tspan,norm_error_state_RK,tspan,norm_error_obs_proj_RK,tspan,norm_error_obs_proj_RK_bf,tspan,error_th,'-k')
...
if all_bf
figure(1)
semilogy(tspan,norm_error_state_RK,'-k',tspan,norm_error_obs_proj_RK,'-or',tspan,norm_error_obs_proj_RK_bf,'-+b',tspan,norm_error_obs_RK_bf2,'--c',tspan,norm_error_obs_RK_bf3,'--m')
...
subplot(2,4,[1 5])
semilogy(tspan(frame_list(1:i)),norm_error_obs_proj_RK(frame_list(1:i)),'-o',tspan(frame_list(1:i)),norm_error_obs_proj_RK_bf(frame_list(1:i)),'-x',tspan(frame_list),error_th(frame_list),'-k')
xlabel('time $t$', 'Interpreter', 'latex','FontSize', fontSize)
ylim([1e-12 1e2])
grid on
...
subplot(2,4,2)
plot(frequences, abs(fft(error_obs_proj_RK(1:Nx,frame))),'o',frequences, abs(fft(error_obs_proj_RK_bf(1:Nx,frame))),'x')
...
subplot(2,4,4)
semilogy(frequences, abs(fft(error_obs_proj_RK(1:Nx,frame))),'o',frequences, abs(fft(error_obs_proj_RK_bf(1:Nx,frame))),'x')
xlim([-floor(Nx/2)-1,floor(Nx/2)+1])
ylim([1e-12,50])
title('frequencies on $\phi$', 'Interpreter', 'latex','FontSize', fontSize)
legend('$P_{BF}(\hat{x} - x)$ (RK4)','$P_{BF}(\hat{x}_{BF} - x)$ (RK4)', 'Interpreter', 'latex','FontSize', fontSize)
...
subplot(2,4,6)
plot(frequences, abs(fft(error_obs_proj_RK(Nx+1:2*Nx,frame))),'o',frequences, abs(fft(error_obs_proj_RK_bf(Nx+1:2*Nx,frame))),'x')
xlim([-floor(Nx/2)-1,floor(Nx/2)+1])
ylim([-2,50])
...
subplot(2,4,8)
semilogy(frequences, abs(fft(error_obs_proj_RK(Nx+1:2*Nx,frame))),'o',frequences, abs(fft(error_obs_proj_RK_bf(Nx+1:2*Nx,frame))),'x')
xlim([-floor(Nx/2)-1,floor(Nx/2)+1])
ylim([1e-12,50])
title('frequencies on $\eta$', 'Interpreter', 'latex','FontSize', fontSize)
legend('$P_{BF}(\hat{x} - x)$ (RK4)','$P_{BF}(\hat{x}_{BF} - x)$ (RK4)', 'Interpreter', 'latex','FontSize', fontSize)
