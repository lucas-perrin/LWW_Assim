...
T        = 20;        % time window
Nif      = 128;         % Number of wave frequency in the solution wave
Nbf      = 2;
...
error_obs_bf2 = vecnorm(PN * (solve_obs_bf - U_mat));

error_bf2     = vecnorm(PN * solve_error_bf);
...
xlim([T_start, 50])
ylim([1e-8, 1e2])
legend('$\|x_{RK4} - x_{C^0}\|$','$\|\hat{x}_{BF} - x_{C^0}\|$','$\|\varepsilon_{BF}\|$', 'Interpreter', 'latex','FontSize', fontSize)
%title(['convergence plots, $U_0 = $ physical values, $N_{if} = $',num2str(Nif),', $N_{bf} = $',num2str(Nbf),', $|\omega| = $',num2str(size_obs)], 'Interpreter', 'latex','FontSize', fontSize)
grid on

subplot(4,4,[11, 12, 15, 16])
semilogy(...
tspan(points_list),error_state(points_list),'-b',...
    tspan(points_list),error_obs_bf2(points_list),'-k',...
    tspan(points_list),error_bf(points_list),'ok')
xlim([T_start, 50])
ylim([1e-8, 1e2])
legend('$\|x_{RK4} - x_{C^0}\|$','$P_{BF}\|\hat{x}_{BF} - x_{C^0}\|$','$P_{BF}\|\varepsilon_{BF}\|$', 'Interpreter', 'latex','FontSize', fontSize)
%title(['convergence plots, $U_0 = $ physical values, $N_{if} = $',num2str(Nif),', $N_{bf} = $',num2str(Nbf),', $|\omega| = $',num2str(size_obs)], 'Interpreter', 'latex','FontSize', fontSize)
grid on
...
xlim([T_start, 50])
ylim([1e-8, 1e2])
legend('$\|x_{RK4} - x_{C^0}\|$','$\|\hat{x}_{BF} - x_{C^0}\|$','$\|\varepsilon_{BF}\|$','$P_{BF}\|\hat{x}_{BF} - x_{C^0}\|$','$P_{BF}\|\varepsilon_{BF}\|$', 'Interpreter', 'latex','FontSize', fontSize)
title(['convergence plots, $U_0 = $ physical values, $N_{if} = $',num2str(Nif),', $N_{bf} = $',num2str(Nbf),', $|\omega| = $',num2str(size_obs)], 'Interpreter', 'latex','FontSize', fontSize)
grid on
